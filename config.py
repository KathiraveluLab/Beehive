...
class Config:
    ...
    CORS_ORIGINS = [
        origin.strip()
        for origin in (_cors_origins_env or "").split(",")
        if origin.strip()
    ]
    ...
    def validate_config():
        ...
        if not Config.JWT_SECRET:
            ...
            errors.append(
                f"JWT_SECRET is set to an insecure default value. Use a strong, unique secret."
            )
        elif len(Config.JWT_SECRET) < 32:
            warnings.append(
                f"JWT_SECRET is too short ({len(Config.JWT_SECRET)} chars). Recommended: 32+ characters."
            )
        ...
        if not Config.SECRET_KEY:
            ...
            errors.append(
                f"FLASK_SECRET_KEY is set to an insecure default value. Use a strong, unique secret."
            )
        elif len(Config.SECRET_KEY) < 32:
            warnings.append(
                f"FLASK_SECRET_KEY is too short ({len(Config.SECRET_KEY)} chars). Recommended: 32+ characters."
            )
        ...
        if not Config.CORS_ORIGINS:
            errors.append(
                "CORS_ORIGINS environment variable is required but not set. Specify allowed origins (comma-separated)."
            )
        ...
        if errors:
            ...
            print(
                "   2. Fill in all required values with secure secrets", file=sys.stderr
            )
            print(
                "   3. Generate strong secrets using: python -c 'import secrets; print(secrets.token_hex(32))'",
                file=sys.stderr,
            )
            print(
