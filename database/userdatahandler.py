...
def save_image(
    id, filename, title, description, time_created, audio_filename=None, sentiment=None
):
    image = {
        "user_id": id,
        "filename": filename,
    ...
    return [
        {
            ...
            "created_at": (
                image.get("created_at").get("$date")
                if isinstance(image.get("created_at"), dict)
                else image.get("created_at")
            ),
        }
        for image in cursor
    ]
...
def _parse_iso_date(date_string, field_name):
    ...
    try:
        ...
        raise ValueError(
            f"Invalid '{field_name}' date format: {date_string}. Expected ISO format."
        )


def search_and_filter_images(
    user_id,
    search_query=None,
    sentiment=None,
    from_date=None,
    ...
    try:
        ...
        if search_query and search_query.strip() and sort_by == "relevance":
            ...
            cursor = beehive_image_collection.find(query, projection).sort(
                [("score", {"$meta": "textScore"})]
            )
        else:
            cursor = beehive_image_collection.find(query, projection).sort(
...
def _get_paginated_images_by_user(user_id, page=1, page_size=12, filters=None):
    ...
    try:
                    ...
                    if filters.get("from"):
                        try:
                            from_date = datetime.fromisoformat(
                                filters["from"].replace("Z", "+00:00")
                            )
                            date_range["$gte"] = from_date
                        except (ValueError, AttributeError):
                            logger.warning(
                                f"Invalid 'from' date format: {filters.get('from')}"
                            )
                    if filters.get("to"):
                        try:
                            to_date = datetime.fromisoformat(
                                filters["to"].replace("Z", "+00:00")
                            )
                            ...
                            logger.warning(
                                f"Invalid 'to' date format: {filters.get('to')}"
                            )
                    if date_range:
        ...
        # Get images
        images = list(
            beehive_image_collection.find(query)
            .sort("created_at", -1)
            .skip(skip)
            .limit(page_size)
        )
...
def update_image(image_id, title, description, sentiment=None):
    ...
    beehive_image_collection.update_one({"_id": image_id}, {"$set": update_data})
...
def get_upload_stats():
    ...
    try:
        ...
        return {"totalImages": 0, "totalVoiceNotes": 0, "totalMedia": 0}
...
def get_recent_uploads(limit=10):
    ...
    try:
        ...
        recent_uploads = list(
            beehive_image_collection.find().sort("created_at", -1).limit(limit)
        )
        if not recent_uploads:
            return []
        # collect user ids and query local user collection
        raw_ids = [
            upload.get("user_id") for upload in recent_uploads if upload.get("user_id")
        ]
        object_ids = []
        for uid in raw_ids:
            try:
        ...
        users_cursor = (
            beehive_user_collection.find({"_id": {"$in": object_ids}})
            if object_ids
            else []
        )
            ...
            uploads_list.append(
                {
                    ...
                    "timestamp": (
                        ...
                        if isinstance(upload.get("created_at"), dict)
                        else upload.get("created_at")
                    ),
...
def get_upload_analytics(trend_days=7):
    try:
        ...
        this_month_count = (
            result["this_month_uploads"][0]["count"]
            if result["this_month_uploads"]
            else 0
        )
        last_month_count = (
            result["last_month_uploads"][0]["count"]
            if result["last_month_uploads"]
            else 0
        )
        increase_perc = (
        ...
        summary = {
            "total": (
                result["total_uploads"][0]["count"] if result["total_uploads"] else 0
            ),
            "voiceNotes": (
                result["voice_notes"][0]["count"] if result["voice_notes"] else 0
            ),
        ...
        for i in range(trend_days):
            ...
            trend.append(
                {"date": date_str, "uploads": {"total": count, "increase": increase}}
            )
            prev_count = count
