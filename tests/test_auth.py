...
def test_change_password(authenticated_client, test_user):
    ...
    # Hash the current password as it would be in the database
    salt = bcrypt.gensalt()
    hashed_password = bcrypt.hashpw(test_user["password"].encode('utf-8'), salt)
...
def test_google_login_callback_admin(client, monkeypatch):
    ...
    monkeypatch.setattr("app.get_google_oauth_token", mock_google_oauth)
...
def test_google_register_submit(client, monkeypatch):
    """Test Google registration submission."""
    # Mock the Google OAuth session
    def mock_google_session():
        return {"email": "newuser@example.com"}
    monkeypatch.setattr("app.get_google_session", mock_google_session)
